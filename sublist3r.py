...
def parse_args():
    ...
    parser.add_argument('-f', '--format', help='Specify format to save (txt|csv)', default='csv')
    parser.add_argument('-F', '--fields', help='Specify fields to save (subdomain,ip,ports)', default='subdomain,ip,ports')
    parser.add_argument('-i', '--findip', help='Find IP address of each subdomain', action='store_true')
    return parser.parse_args()


def write_file(filename, fileformat, fields, subdomain, ip=None, ports=None):
    ...
    with open(str(filename), 'a') as f:
        if fileformat == 'txt':
            if fields == 'ip':
                ...
                data = "{}".format(subdomain)
                if ip:
                    data += " ({})".format(ip) 
                if ports:
                    data += " - Found open ports: {}".format(ports)
            f.write(data + "\n")
        elif fileformat == 'csv':
            if ip == '0.0.0.0':
                ip = ''
            if ports:
                row = (subdomain, ip, ",".join(ports))
            else:
                row = (subdomain, ip)
...
class Portscan():
    def __init__(self, subdomains, ports, savefile, fileformat, fields, find_ip=False):
    ...
    def port_scan(self, subdomain):
        ...
        if len(open_ports) > 0:
            if self.find_ip:
                ...
                if self.savefile:
                    write_file(self.savefile, self.fileformat, self.fields, 
                               subdomain, ip, open_ports)
            else:
...
def main(domain, threads, savefile, fileformat, fields, ports, silent, verbose, enable_bruteforce, engines, find_ip):
    bruteforce_list = set()
    search_list = set()
    ...
    if savefile:
        ...
        if fileformat == 'csv':
            ...
            print("%s[-] Fieldnames to save: %s%s%s%s" % (Y, W, R, fieldnames, W))
            with open(savefile, 'wb') as csvfile:
                subwriter = csv.writer(csvfile)
                subwriter.writerow(fieldnames)
    ...
    if subdomains:
        ...
        if ports:
            ...
            ports = ports.split(',')
            pscan = Portscan(subdomains, ports, savefile, fileformat, fields, find_ip)
            pscan.run()
            ...
            for subdomain in subdomains:
                if find_ip:
                    ...
                    print("{}{} ({}){}".format(G, subdomain, ip, W))
                    if savefile:
                        write_file(savefile, fileformat, fields, subdomain, ip)
                else:
                    print("{}{}{}".format(G, subdomain, W))
                    if savefile:
                        write_file(savefile, fileformat, fields, subdomain)
...
if __name__ == "__main__":
    ...
    if verbose or verbose is None:
        verbose = True
    banner()
    res = main(domain, threads, savefile, fileformat, fields, ports, silent=False, verbose=verbose, enable_bruteforce=enable_bruteforce, engines=engines, find_ip=find_ip)
